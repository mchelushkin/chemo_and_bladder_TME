---
title: Correlations between signatures, distances, and PD-L1. Comparing independent Rs.
...
all_dens = norm_log_dens.loc[:, dens_columns]
```
...
#ssgsea_h = ssgsea_h.reset_index().assign(sample_type = ssgsea_h.reset_index().loc[:, 'index'].apply(lambda x: x.split(' ')[1])).assign(pt_number = ssgsea_h.reset_index().loc[:, 'index'].apply(lambda x: x.split(' ')[0])).set_index('index')
```
...
```{python, fig.width = 10.5, fig.height = 10}
plt.rcParams.update({'font.size': 12})
box_palette = {'high': 'bisque', 'low': 'powderblue'}
cmap_reversed = matplotlib.cm.get_cmap('RdYlBu_r')
plt.close()
...
af = axs.flat
ht_pearson(df_delta, af[0], mult_corr=False, cbar = True)
ht_pearson(df_cys, af[1], mult_corr=True, cbar = True)
...
_ = sns.heatmap(df, cmap = cmap_reversed, annot = df, yticklabels = True, ax=af[3], vmin=0, vmax=0.1, alpha=0.6)
af[3].set_xlabel('')
af[3].set_ylabel('')
af[3].set_yticks([])
af[3].set_yticklabels([])
       
af[0].set_yticklabels(tick_list)
af[2].set_yticklabels(tick_list)
...
af[3].set_xticklabels(['Fibroblast TGFb', 'TNFa-via-NFkB', 'PD-L1 IC'])
