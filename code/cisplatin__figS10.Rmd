---
title: Cisplatin-treated sub-cohort. Distance analysis in dynamics.
...
all_med.columns = all_med.columns.str.replace('CD4_Tcell', 'T_helper')
...
```{python}
diff_dist = np.log2(cys_med.reset_index().replace({'sample': {' Cystectomy': ''}}, regex=True).set_index('sample')) - np.log2(tur_med.reset_index().replace({'sample': {' TUR': ''}}, regex=True).set_index('sample'))
```

```{r}
for (varb in colnames(py$diff_dist)) {
  print(varb)
  print(identify_outliers(py$diff_dist, variable = varb)$is.extreme)}
```

```{python}
sh_res = {}
for x in diff_dist.columns:
  sh_res.update({x: shapiro(diff_dist.loc[:, x].dropna()).pvalue})
sh_res = pd.Series(sh_res)
```
...
cell_perc = cell_perc.loc[cell_perc.index.isin(plt_list)]
...
```{r}
identify_outliers(py$df_fc, variable = 'CD8_Tcell_to_tumor_median')
py$df_fc %>% shapiro_test(CD8_Tcell_to_tumor_median)
```

```{r}
identify_outliers(py$df_fc, variable = 'CD8_Tcells')
py$df_fc %>% shapiro_test(CD8_Tcells)
```
...
norm_log_dens = norm_log_dens.loc[norm_log_dens.index.isin(plt_list)]
...
```{r}
for (varb in colnames(py$diff_dens)) {
  print(varb)
  print(identify_outliers(py$diff_dens, variable = varb)$is.extreme)}
```
```{python}
sh_res = {}
for x in diff_dens.columns:
  sh_res.update({x: shapiro(diff_dens.loc[:, x].dropna()).pvalue})
sh_res = pd.Series(sh_res)
```
```{python}
sh_res
```
```{r}
#ggqqplot(py$diff_dens, "Negative_cells")
```
...
```{r}
fr <- readRDS('../data/cisplatin_diffexpression_fgsea_res.rds')
```
